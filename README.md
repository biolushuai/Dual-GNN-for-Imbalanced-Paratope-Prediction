...
# Abstract
Accurate prediction of antibody paratopes is critical for elucidating immune recognition mechanisms and accelerating the discovery of therapeutic antibodies. Existing deep-learning predictors are limited by severe class imbalance in benchmark datasets, the computational cost of full fine-tuning for large protein language models (PLMs), and the underuse of 3D structural information. We propose a framework that addresses these issues from both sequence and structure perspectives. For sequence-based modeling, we develop ParaLoRA, a lightweight parameter-efficient fine-tuning (PEFT) method that incorporates Low-Rank Adaptation (LoRA) into PLMs. ParaLoRA enables task adaptation for paratope prediction with only a small number of additional trainable parameters. It further mitigates class imbalance via targeted sampling and cost-sensitive weighting. For structure-based prediction, we design ParaDG, a dual-view graph learning model. ParaDG constructs a global-view GCN over all residues and a surface-view GAT over solvent-exposed residues, initialized with ParaLoRA embeddings and trained with a structure-preserving graph oversampler. Both ParaLoRA and ParaDG operate under antigen-free conditions, requiring only antibody sequence or structure as input, making them suitable for high-throughput pre-screening of antibody libraries when complex structures are unavailable. ParaLoRA achieves highly competitive performance with only 0.09\% trainable parameters, and ParaDG establishes new state-of-the-art AUC-ROC and AUC-PR among structure-based predictors. Code and preprocessed data are available at https://github.com/biolushuai/Dual-GNN-for-Imbalanced-Paratope-Prediction.
